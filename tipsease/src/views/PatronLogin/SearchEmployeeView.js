...
class SearchEmployeeView extends React.Component {
  ...
  search = e => {
    const searchedEmployee = this.props.employeeList.filter(employee => {
      return (
        (employee.first_name.toLowerCase().includes(e.target.value) ||
          employee.last_name.toLowerCase().includes(e.target.value)) &&
        employee
      );
    });
  ...
  render() {
    return (
      <div>
        {this.props.isFetching && <h4>Wait while we fetch your data...</h4>}
